      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-bold text-brand-text dark:text-white">
              Continue Learning
            </h2>
            <Link
              href="/my-learning"
          ...
          {inProgressCourses.length === 0 ? (
            <div className="p-8 text-center rounded-[14px] border border-brand-border dark:border-brand-darkBorder bg-white dark:bg-brand-darkSoft space-y-3">
            ...
            <div className="space-y-3">
              {inProgressCourses.map((course) => (
                <div
                  key={course.id}
                  className="p-5 rounded-[14px] border border-brand-border dark:border-brand-darkBorder bg-white dark:bg-brand-darkSoft hover:shadow-sm transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-4"
                >
                  <div className="flex-1 min-w-0">
                    <span className="text-[11px] font-semibold text-brand-muted dark:text-brand-darkText/60 uppercase tracking-wider">
                      {course.category}
                    </span>
                    <h3 className="text-base font-bold text-brand-text dark:text-white truncate mt-0.5">
                      {course.title}
                    </h3>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {upcomingClasses.map((cls) => (
              <div
                key={cls.id}
                className="p-4 rounded-[14px] border border-brand-border dark:border-brand-darkBorder bg-white dark:bg-brand-darkSoft hover:shadow-sm transition-all"
              >
                <div className="flex items-start justify-between gap-2">
                  ...
                  <span className="text-[11px] px-2 py-0.5 rounded-full font-medium bg-brand-soft dark:bg-brand-darkBg text-brand-navy dark:text-white border border-brand-border dark:border-brand-darkBorder shrink-0">
                    {cls.date}
                  </span>
                </div>

                <div className="mt-3 pt-2.5 border-t border-brand-border/60 dark:border-brand-darkBorder/60 flex items-center justify-between gap-2">
                  <span className="text-xs text-brand-muted dark:text-brand-darkText/60">
                    {cls.duration}
                  </span>
                  {cls.attended ? (
                    ...
                    <button
                      ...
                      className="px-3 py-1 rounded-full bg-brand-focus hover:bg-brand-focus/90 text-white text-xs font-semibold transition-colors"
                    >
                      Join Class
                    </button>
                  )}
