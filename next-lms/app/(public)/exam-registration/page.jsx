...
const ExamRegistrationPage = () => {
  ...
  const fetchStudentSuggestions = async (searchTerm) => {
    ...
    try {
      const response = await fetch(
        `/api/students/search/autocomplete?query=${encodeURIComponent(searchTerm)}`
      );
      const data = await response.json();
  ...
  const validateStudentId = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/students/search/autocomplete?query=${encodeURIComponent(studentIdInput.trim())}`
      );
      const data = await response.json();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        const updatedResponse = await fetch(`/api/exams`);
  ...
  return (
    <div className={styles['exam-registration-page'] || 'exam-registration-page'}>
      <Header onSearch={handleSearch} />
      
      <main className={styles['exam-registration-main'] || 'exam-registration-main'}>
        <div className={styles['exam-registration-content'] || 'exam-registration-content'}>
          <div className={styles['exam-registration-header'] || 'exam-registration-header'}>
            <h1>Exam Registration</h1>
            <div className={styles['exam-registration-header-actions'] || 'exam-registration-header-actions'}>
          ...
          {existingExamRecord && (
            <div className={styles['exam-form-container'] || 'exam-form-container'}>
              ...
              <div className={styles['existing-exam-record'] || 'existing-exam-record'}>
                <div className={styles['success-message'] || 'success-message'} style={{ marginBottom: '1rem' }}>
                  This student already has an exam registration record.
                </div>
                <div className={styles['exam-record-details'] || 'exam-record-details'}>
                  <div className={styles['detail-row'] || 'detail-row'}>
                    <span className={styles['detail-label'] || 'detail-label'}>Student ID:</span>
                    <span className={styles['detail-value'] || 'detail-value'}>{existingExamRecord.studentIdNumber || existingExamRecord.studentId?.studentId || '-'}</span>
                  </div>
                  <div className={styles['detail-row'] || 'detail-row'}>
                    <span className={styles['detail-label'] || 'detail-label'}>Student Name:</span>
                    <span className={styles['detail-value'] || 'detail-value'}>{existingExamRecord.studentId?.name || existingExamRecord.otherNames + ' ' + existingExamRecord.familyName || '-'}</span>
                  </div>
                  <div className={styles['detail-row'] || 'detail-row'}>
                    <span className={styles['detail-label'] || 'detail-label'}>Exam:</span>
                    <span className={styles['detail-value'] || 'detail-value'}>
                      {existingExamRecord.exams && Array.isArray(existingExamRecord.exams) && existingExamRecord.exams.length > 0
                        ? existingExamRecord.exams.map((ex, idx) => {
                            ...
                            return <span key={idx} className={styles['exam-badge'] || 'exam-badge'} style={{ marginRight: '0.5rem' }}>{subjectName}</span>;
                          })
                  ...
                  <div className={styles['detail-row'] || 'detail-row'}>
                    <span className={styles['detail-label'] || 'detail-label'}>Exam Date:</span>
                    <span className={styles['detail-value'] || 'detail-value'}>{existingExamRecord.examDate ? formatDate(existingExamRecord.examDate) : '-'}</span>
                  </div>
                  <div className={styles['detail-row'] || 'detail-row'}>
                    <span className={styles['detail-label'] || 'detail-label'}>Title:</span>
                    <span className={styles['detail-value'] || 'detail-value'}>{existingExamRecord.title || '-'}</span>
                  </div>
                  <div className={styles['detail-row'] || 'detail-row'}>
                    <span className={styles['detail-label'] || 'detail-label'}>Other Names:</span>
                    <span className={styles['detail-value'] || 'detail-value'}>{existingExamRecord.otherNames || existingExamRecord.firstName || '-'}</span>
                  </div>
                  <div className={styles['detail-row'] || 'detail-row'}>
                    <span className={styles['detail-label'] || 'detail-label'}>Family Name:</span>
                    <span className={styles['detail-value'] || 'detail-value'}>{existingExamRecord.familyName || existingExamRecord.lastName || '-'}</span>
                  </div>
                  <div className={styles['detail-row'] || 'detail-row'}>
                    <span className={styles['detail-label'] || 'detail-label'}>Email:</span>
                    <span className={styles['detail-value'] || 'detail-value'}>{existingExamRecord.email || existingExamRecord.studentId?.email || '-'}</span>
                  </div>
                  <div className={styles['detail-row'] || 'detail-row'}>
                    <span className={styles['detail-label'] || 'detail-label'}>Date of Birth:</span>
                    <span className={styles['detail-value'] || 'detail-value'}>
                      {(() => {
                  ...
                  <div className={styles['detail-row'] || 'detail-row'}>
                    <span className={styles['detail-label'] || 'detail-label'}>Gender:</span>
                    <span className={styles['detail-value'] || 'detail-value'}>{existingExamRecord.gender || existingExamRecord.studentId?.gender || '-'}</span>
                  </div>
                  <div className={styles['detail-row'] || 'detail-row'}>
                    <span className={styles['detail-label'] || 'detail-label'}>Mobile:</span>
                    <span className={styles['detail-value'] || 'detail-value'}>{existingExamRecord.mobile || existingExamRecord.studentId?.mobile || '-'}</span>
                  </div>
                  <div className={styles['detail-row'] || 'detail-row'}>
                    <span className={styles['detail-label'] || 'detail-label'}>Telephone:</span>
                    <span className={styles['detail-value'] || 'detail-value'}>{existingExamRecord.telephone || '-'}</span>
                  </div>
                  <div className={styles['detail-row'] || 'detail-row'}>
                    <span className={styles['detail-label'] || 'detail-label'}>Candidate ID Number:</span>
                    <span className={styles['detail-value'] || 'detail-value'}>{existingExamRecord.candidateIdNumber || '-'}</span>
                  </div>
                </div>
                <div className={styles['form-actions'] || 'form-actions'} style={{ marginTop: '1rem' }}>
                  <button 
                    type="button" 
                  ...
                  <div className={styles['student-id-validation'] || 'student-id-validation'}>
                    <div className={styles['student-search-container'] || 'student-search-container'} style={{ position: 'relative', width: '100%' }}>
                      ...
                      {showSuggestions && studentSuggestions.length > 0 && (
                        <div className={styles['student-suggestions-dropdown'] || 'student-suggestions-dropdown'}>
                          {studentSuggestions.map((student, index) => (
                            <div
                              key={student._id || index}
                              onClick={() => handleStudentSuggestionSelect(student)}
                              className={styles['suggestion-item'] || 'suggestion-item'}
                            >
                    ...
                    {filteredExams.map((exam) => {
                      ...
                      return (
                        <tr key={exam._id}>
                          ...
                          <td className={styles['student-name-cell'] || 'student-name-cell'}>{fullName}</td>
                          ...
                          <td>
                            <span className={`${styles[(styles['status-badge'] || 'status-badge')] || (styles['status-badge'] || 'status-badge')} ${styles[(styles['registered'] || 'registered')] || (styles['registered'] || 'registered')}`}>Registered</span>
                          </td>
                        </tr>
                      );
                    })}
