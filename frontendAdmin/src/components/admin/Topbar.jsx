...
const Topbar = ({ userName }) => {
  ...
  // Bumped whenever the profile page saves changes, so the avatar/name refresh live
  const [profileVersion, setProfileVersion] = useState(0);

  const notificationsRef = useRef(null);
  const profileRef = useRef(null);

  // User details from localStorage or props (re-read on every render/profileVersion bump)
  void profileVersion;
    ...
    <header className="topbar">
      ...
      <div className="topbar-right-group">
        ...
        <div className="topbar-action-item" ref={profileRef}>
          ...
          >
            {/* Circular Gradient Avatar with Initials, or the real profile picture */}
            {storedUser.profileImage ? (
              <img src={getImageUrl(storedUser.profileImage)} alt="Profile" className="profile-avatar-gradient profile-avatar-img" />
            ) : (
          ...
          {isProfileOpen && (
            <div className="topbar-dropdown profile-dropdown-panel" role="menu" aria-label="User profile options">
              <div className="profile-dropdown-top">
                {storedUser.profileImage ? (
                  <img src={getImageUrl(storedUser.profileImage)} alt="Profile" className="profile-avatar-gradient large profile-avatar-img" />
                ) : (
              ...
              <div className="dropdown-action-list">
                ...
                <button
                  ...
                  onClick={() => {
                    setIsProfileOpen(false);
                    navigate('/admin/profile', { state: { tab: 'institute' } });
                  }}
                  role="menuitem"
                >
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
