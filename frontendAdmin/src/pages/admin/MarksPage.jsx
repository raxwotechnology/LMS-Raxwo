...
const MarksPage = () => {
  ...
  // ── Paper Submissions state ────────────────────────────────
  const [activeTab, setActiveTab] = useState('marks'); // 'marks' | 'submissions'
          ...
          <div className="marks-header">
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', gap: '4px', background: '#f1f5f9', borderRadius: '10px', padding: '4px' }}>
                ...
                >
                  📄 Paper Submissions {paperSubmissions.length > 0 && <span style={{ background: '#6366f1', color: '#fff', borderRadius: '20px', padding: '0 7px', fontSize: '0.7rem', marginLeft: '4px' }}>{paperSubmissions.length}</span>}
                </button>
              </div>
            </div>
            {activeTab === 'marks' && (
                    ...
                    <thead>
                      <tr style={{ background: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
                        {['Student ID', 'Student Name', 'Paper Title', 'Subject', 'Grade', 'Marks', '%', 'Result', 'Submitted', 'Action'].map(h => (
                          <th key={h} style={{ padding: '10px 12px', textAlign: 'left', fontSize: '0.75rem', fontWeight: '700', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.04em', whiteSpace: 'nowrap' }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {paperSubmissions.map((sub, i) => (
                        <tr key={sub._id} style={{ borderBottom: '1px solid #f1f5f9', background: i % 2 === 0 ? '#fff' : '#fafafa' }}>
                          <td style={{ padding: '10px 12px', fontSize: '0.82rem', fontWeight: '600', color: '#374151' }}>{sub.studentId}</td>
                          <td style={{ padding: '10px 12px', fontSize: '0.82rem', color: '#374151' }}>{sub.studentName || '—'}</td>
                          <td style={{ padding: '10px 12px', fontSize: '0.82rem', color: '#374151', maxWidth: '180px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{sub.paperTitle}</td>
                          <td style={{ padding: '10px 12px', fontSize: '0.82rem', color: '#374151' }}>{sub.subject}</td>
                          <td style={{ padding: '10px 12px', fontSize: '0.82rem', color: '#374151' }}>{sub.grade || '—'}</td>
                          <td style={{ padding: '10px 12px', fontSize: '0.82rem', fontWeight: '700', color: '#1e293b' }}>{sub.earnedMarks}/{sub.totalMarks}</td>
                          ...
                          <td style={{ padding: '10px 12px' }}>
                            <button
                              ...
                              style={{ background: '#6366f1', color: '#fff', border: 'none', borderRadius: '7px', padding: '5px 14px', fontWeight: '600', fontSize: '0.78rem', cursor: 'pointer', whiteSpace: 'nowrap' }}
                            >
                              👁 View Paper
                            </button>
                          </td>
          ...
          <div className="marks-modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '820px', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ padding: '16px 20px' }}>
              ...
              <div style={{
                ...
                borderRadius: '10px', border: `1px solid ${viewingSubmission.passed ? '#86efac' : '#fca5a5'}`,
                alignItems: 'center'
              }}>
                <div style={{ textAlign: 'center' }}>
                  <div style={{ fontSize: '1.6rem', fontWeight: '800', color: '#1e293b' }}>{viewingSubmission.earnedMarks}/{viewingSubmission.totalMarks}</div>
                  <div style={{ fontSize: '0.7rem', color: '#64748b', fontWeight: '600', letterSpacing: '0.05em' }}>MARKS</div>
                </div>
                <div style={{ textAlign: 'center' }}>
                  <div style={{ fontSize: '1.6rem', fontWeight: '800', color: '#6366f1' }}>{viewingSubmission.percentage}%</div>
                  <div style={{ fontSize: '0.7rem', color: '#64748b', fontWeight: '600', letterSpacing: '0.05em' }}>PERCENTAGE</div>
                </div>
                <div style={{ textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: '0.7rem', color: '#64748b', fontWeight: '600', letterSpacing: '0.05em' }}>RESULT</div>
                </div>
                <div style={{ textAlign: 'center' }}>
                  <div style={{ fontSize: '1.2rem', fontWeight: '700', color: '#374151' }}>{viewingSubmission.passMark}</div>
                  <div style={{ fontSize: '0.7rem', color: '#64748b', fontWeight: '600', letterSpacing: '0.05em' }}>PASS MARK</div>
                </div>
                <div style={{ textAlign: 'center', marginLeft: 'auto' }}>
                  <div style={{ fontSize: '0.85rem', fontWeight: '600', color: '#374151' }}>
                    {new Date(viewingSubmission.submittedAt).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}
                  </div>
                  <div style={{ fontSize: '0.7rem', color: '#64748b', fontWeight: '600', letterSpacing: '0.05em' }}>SUBMITTED</div>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {(viewingSubmission.answers || []).map((a, idx) => {
                  const isMCQorTF = a.questionType === 'MCQ' || a.questionType === 'True/False';
                  const hasCorrAns = !!a.correctAnswer;
                  return (
                    <div key={idx} style={{
                      border: `1.5px solid ${isMCQorTF && hasCorrAns ? (a.isCorrect ? '#86efac' : '#fca5a5') : '#e2e8f0'}`,
                      borderRadius: '10px', padding: '12px 16px',
                      background: isMCQorTF && hasCorrAns ? (a.isCorrect ? '#f0fdf4' : '#fff5f5') : '#fafafa'
                    }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '8px' }}>
                        <div style={{ display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
                          <span style={{
                            minWidth: '26px', height: '26px', borderRadius: '50%', background: '#6366f1',
                            color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center',
                            fontSize: '0.72rem', fontWeight: '800', flexShrink: 0
                          }}>Q{a.questionNumber}</span>
                          <div>
                            <div style={{ fontWeight: '600', fontSize: '0.88rem', color: '#1e293b' }}>{a.questionText}</div>
                            <div style={{ fontSize: '0.7rem', color: '#94a3b8', marginTop: '2px' }}>{a.questionType}</div>
                          </div>
                        </div>
                        <div style={{ display: 'flex', gap: '5px', flexShrink: 0 }}>
                          {isMCQorTF && hasCorrAns && (
                            <span style={{ fontSize: '0.72rem', fontWeight: '800', padding: '2px 8px', borderRadius: '20px', background: a.isCorrect ? '#dcfce7' : '#fee2e2', color: a.isCorrect ? '#15803d' : '#b91c1c' }}>
                              {a.isCorrect ? '✓ Correct' : '✗ Wrong'}
                            </span>
                          )}
                          <span style={{ fontSize: '0.72rem', fontWeight: '700', padding: '2px 8px', borderRadius: '20px', background: '#eff6ff', color: '#1d4ed8' }}>
                            {a.earnedMarks}/{a.totalMarks} mk
                          </span>
                        </div>
                      </div>
                      <div style={{ marginTop: '8px', paddingLeft: '36px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                        <div style={{ fontSize: '0.83rem', color: '#475569' }}>
                          <strong>Student's Answer: </strong>
                          <span style={{ color: a.isCorrect ? '#15803d' : (hasCorrAns && a.studentAnswer ? '#b91c1c' : '#374151'), fontWeight: '600' }}>
                            {a.studentAnswer || <em style={{ color: '#94a3b8', fontWeight: 'normal' }}>No answer given</em>}
                          </span>
                        </div>
                        {hasCorrAns && (
                          <div style={{ fontSize: '0.83rem', color: '#475569' }}>
                            <strong>Correct Answer: </strong>
                            <span style={{ color: '#15803d', fontWeight: '600' }}>{a.correctAnswer}</span>
                          </div>
                        )}
                      </div>
                    </div>
